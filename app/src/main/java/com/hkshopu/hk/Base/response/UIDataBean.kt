...
class UIDataBean<T>(var status: Status, var ret_val: T?, var code: Int?, var e: Throwable?) {
    ...
    //只有请求状态和数据的UIData构造
    constructor(status: Status, ret_val: T?) : this(status, ret_val, -1, null)

    //请求成功且数据正确返回的UIData构造
    constructor(ret_val: T?) : this(Status.Success, ret_val, -1, null)

    //请求失败的UIData构造，包含网络异常，一般错误，接口返回异常等
    constructor(e: Throwable?) : this(Status.Error, null, -1, e)

    //请求成功后需要拿当前返回的code
    constructor(ret_val: T?, code: Int?) : this(Status.Success, ret_val, code, null)
