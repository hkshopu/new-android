...
class ShopmanageRepository : BaseRepository(){
    ...
    fun getShopRecommend(lifecycleOwner: LifecycleOwner,userId:Int,mode:String,max_seq:Int) : Observable<ListBean<ShopRecommendBean>> {
        return service.getShopRecommend(userId,mode, max_seq)
            .compose(SchedulersUtil.applySchedulers())
            .bindUntilEvent(lifecycleOwner,Lifecycle.Event.ON_DESTROY)
            .compose(handleBean())
            .map {
                val list = ListBean<ShopRecommendBean>()
                list
            }
